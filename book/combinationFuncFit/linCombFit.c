...
int main(void)
{
    ...
    printf("Type the number of functions in the linear combination (maximum %d) : ", NUM_EQUATIONS);
    ...
    gauss( m, a);
    printf("\n!!!Solved!!!!");
    printf( "\n\n Function coefficients determined \n");
    printf( "-------------------------------------\n");
...
double x_ini[]; double y_ini[];
{
    ...
    while(currentPoint <= upperBound)
    {
        ...
        for( i = 0; i < numCoef; i++)
        {
            yans += coefficients[i]*fun(i,currentPoint);
        }
    ...
    numCommands++;
    char * output = "set terminal png \nset output \"linCombFitExpDrift.png\" \nunset key";
    numCommands++;
